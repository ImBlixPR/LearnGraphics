...
namespace Game
{
    ...
    struct Fruit
    {
        ...
        Fruit() : object(nullptr), active(false) {}
        ~Fruit()
        {
            if (object)
            {
    ...
    void SpawnFruit() {
        ...
        // Create new object
        if (typeOfFruit) {
            fruit.object = new Object("res/textures/fruit1.png", position, 0.0f, 50.0f, 50.0f);
        }
        else {
            fruit.object = new Object("res/textures/fruit2.png", position, 0.0f, 50.0f, 50.0f);
        }
    ...
    void Clear() {
        ...
        for (int i = 0; i < FRUIT_POOL_SIZE; i++) {
            if (fruitPool[i].object) {
                delete fruitPool[i].object;
                fruitPool[i].object = nullptr;
            }
            fruitPool[i].active = false;
        }
